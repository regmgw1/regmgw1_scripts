...
=head2 Description
produces files of first last inter exons & introns.
=head2 Usage
...
unless (@ARGV ==3) {
        die "\n\nUsage:\n ./first_exon_last_exon_v3.pl path2exons path2output gene_select?(0|path2list)\nPlease try again.\n\n\n";}
...
while (my $line = <IN>)
{
	...
	my $start = $elems[3];
	my $stop = $elems[4];
	if ($elems[2] eq "X" or $elems[2] eq "Y")
	{
		next;
	}
	my $ok = 1;
	...
	if ($ok == 1)
	{
		my $gff = "$elems[2]\t$elems[0]"."_$elems[1]\tchr$elems[2]".":$elems[3]"."-$elems[4]\t$elems[3]\t$elems[4]\t$elems[7]\t.\t$elems[5]\tBiomart52";
		...
		if ($trans_id eq $old_id)
		{
			if ($ex_pos == 1)
			{
				...
				{
					$first_intron_start = $start - 1;
				}
				$first_intron_stop = intron_find($first_intron_start, $elems[2], $elems[7]);
				intron_print($first_intron_start,$first_intron_stop,$elems[2], $elems[7], 1)
			}
			elsif ($ex_pos > $max_pos)
			{
				...
				if ($max_line ne "0")
				{
					...
					my $intron_stop = intron_find($intron_start, $elems[2], $elems[7]);
					intron_print($intron_start,$intron_stop,$elems[2], $elems[7], 0)
...
sub intron_find
{
	...
	my $sth1 = $dbh->prepare("SELECT $tmpstop FROM introns where chr = '$chr' and $tmpstart = $intron_start")
...
sub intron_print
{
	...
	if ($intron_stop ne "0")
	{
		if ($first == 1)
		{
			if ($strand == 1)
			{
				print FINT "$chr\tFirst_Intron\tchr$chr".":$intron_start"."-$intron_stop\t$intron_start\t$intron_stop\t$strand\t.\t.\tBiomart52\n";
			}
			elsif ($strand == -1)
			{
				print FINT "$chr\tFirst_Intron\tchr$chr".":$intron_stop"."-$intron_start\t$intron_stop\t$intron_start\t$strand\t.\t.\tBiomart52\n";
			}	
		}
		else
		{
			if ($strand == 1)
			{
				print INTRON "$chr\tIntron\tchr$chr".":$intron_start"."-$intron_stop\t$intron_start\t$intron_stop\t$strand\t.\t.\tBiomart52\n";
			}
			elsif ($strand == -1)
			{
				print INTRON "$chr\tIntron\tchr$chr".":$intron_stop"."-$intron_start\t$intron_stop\t$intron_start\t$strand\t.\t.\tBiomart52\n";
			}	
		}
	}
}
