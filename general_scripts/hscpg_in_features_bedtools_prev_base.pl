...
Usage: /hscpg_in_features_bedtools_prev_base.pl path2hscpgs path2featureList path2features max_cluster_number path2output

=cut

#################################################################
# hscpg_in_features_bedtools_prev_base.pl
#################################################################

use strict;

unless (@ARGV ==6) {
        die "\n\nUsage:\n ./hscpg_in_features_bedtools_prev_base.pl path2hscpgs path2featureList path2features max_peak_size prevBase path2output\nPlease try again.\n\n\n";}
...
my (%hash, %prevHash);
$prevBase = lc($prevBase);
my $pCount = 0;

open (PREV, "/path2/hscpg_extended_140211/human_primates_cg_140211.txt" ) or die "Can't open prev for reading";
while (my $prev = <PREV>)
{
	if ($pCount > 0)
	{
...
for (my $i = 1;$i<=$maxPeak;$i++)
{
	...
	while (my $dmr = <DMR>)
	{
		...
		else
		{
			...
			{
				my $coords = $elems[0]."_".$elems[1];
				if (exists $prevHash{$coords})
				{
					if ($prevHash{$coords} eq $prevBase)
					{
						print TMP "$dmr";
					}
				}
